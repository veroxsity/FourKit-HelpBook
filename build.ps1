#requires -Version 5
# Build HelpBook and deploy to the server plugins folder.
...
& $dotnet build (Join-Path $plug 'HelpBook.csproj') -c Release
if ($LASTEXITCODE -ne 0) { throw "Build failed (exit $LASTEXITCODE)" }

$built  = Join-Path $plug 'bin\Release\net10.0\HelpBook.dll'
$target = Join-Path $server 'plugins\HelpBook.dll'
